...
function setup() {
    ...
    mainc = createSprite(50, 10, 40, 40);
    mainc.addImage("player", mainc_IMG);
    mainc.velocityY = -3; 
    mainc.scale = 0.5; 

    gameOver = createSprite(30, 250, 20, 20);
    gameOver.addImage("done", gameOverImg);
    gameOver.visible = false;

    restart = createSprite(30, 300, 20, 20);
...
function points() {
    if(frameCount % 70 === 0){
        var coin = createSprite(20, 70, 50, 50);
        coin.scale = 0.5;
...
function superpoints(){
    if(frameCount % 200 === 0){
        var supercoin = createSprite(20, 70, 70, 70);
        supercoin.scale = 0.5;
...
function obstacles() {
    if(frameCount % 30 === 0) {
        var obstacles = createSprite(60, 60, 50, 50);
        obstacles.scale = 1.5;
        ...
        switch(rand) {
          case 1: obstacles.addImage("obsatcle1", obstacle1_IMG);
                  break;
          case 2: obstacles.addImage("obsatcle2", obstacle2_IMG);
                  break;
          case 3: obstacles.addImage("obsatcle3", obstacle3_IMG);
                  break;
          default: break;
        }
